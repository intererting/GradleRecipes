...
import com.example.normal.gitmanifest.VerifyManifestTask
...
fun ApplicationAndroidComponentsExtension.configure(project: Project) {
    ...
    onVariants(selector().withBuildType("release")) { variant ->
        ...
        val versionCodeTask = project.tasks.register(
            "computeVersionCodeFor${variant.name}",
        ...
        mainOutput.versionCode.set(versionCodeTask.flatMap {
            it.outputFile.map {
        ...
        val versionNameTask = project.tasks.register(
            "computeVersionNameFor${variant.name}",
